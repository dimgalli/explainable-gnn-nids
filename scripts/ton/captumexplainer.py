...
def to_graph(data):
    G = nx.from_pandas_edgelist(data, source='src_ip_port', target='dst_ip_port', edge_attr=['eid', 'x', 'label'], create_using=nx.MultiGraph())
    G = G.to_directed()
...
parser = argparse.ArgumentParser(description='Train and test CaptumExplainer model')
...
parser.add_argument('--scores', type=str, required=True, help='path to save the CaptumExplainer model scores')
...
test_data.insert(38, 'eid', test_data.index)
